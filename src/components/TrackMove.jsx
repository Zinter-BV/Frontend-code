    ...
    return (
      <div className="ml-4 movingCompanyDetailBox h-fit w-full">
        <div className="flex items-center justify-center h-64">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#136AB5]"></div>
          <span className="ml-2 text-[#136AB5]">Loading tracking data...</span>
        </div>
      </div>
    );
  }
...
//   // React Query hook with 5-second refetch interval
//   const { data, isLoading, error } = useQuery({
//     queryKey: ["trackMove", moversData?.quoteId],
//     queryFn: fetchTrackingData,
//     refetchInterval: 5000, // Refetch every 5 seconds
//     refetchIntervalInBackground: true, // Continue refetching when tab is not active
//     retry: 3, // Retry failed requests up to 3 times
//     staleTime: 0, // Consider data stale immediately
//     enabled: !!moversData?.quoteId, // Only run query if quoteId exists
//   });

//   // Function to calculate progress percentages based on API response
//   const getProgressPercentages = () => {
//     if (!data?.result) return { top: 100, bottom: 0 };

//     const { hasArrived, inTransit, isCompleted } = data.result;

//     if (isCompleted) {
//       return { top: 0, bottom: 100 }; // 0% - 100%
//     } else if (inTransit) {
//       return { top: 25, bottom: 75 }; // 25% - 75%
//     } else if (hasArrived) {
//       return { top: 50, bottom: 50 }; // 50% - 50%
//     } else {
//       return { top: 75, bottom: 25 }; // 75% - 25% (initial state)
//     }
//   };

//   const { top, bottom } = getProgressPercentages();
...
//               ></div>
//               <div
//                 className="w-full bg-red-500 opacity-90 transition-all duration-500 ease-in-out"
//                 style={{ height: `${bottom}%` }}
//               ></div>
//             </div>
//             <p className="font-sans text-[18px] mb-2 font-extralight text-[#121212] ">
